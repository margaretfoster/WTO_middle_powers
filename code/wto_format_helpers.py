...
def save_table(df, title, fname, digits=3, bold_sig=True):
    """
    Save a pandas DataFrame as a formatted Word table.

    Args:
        df: pandas DataFrame
        title: str, table title
        fname: str, output .docx filename
        digits: int, number of decimal places for numeric values
        bold_sig: bool, bold p-values < .05
    """
    doc = Document()
    ...
    for _, row in df.iterrows():
        ...
        for i, val in enumerate(row):
            ...
            if isinstance(val, (int, float, np.number)):
                cell_text = f"{val:.{digits}f}"
            else:
                cell_text = str(val)
            p = row_cells[i].paragraphs[0]
            run = p.add_run(cell_text)

            # Optional bolding for significant p-values
            if bold_sig and "p" in df.columns[i].lower():
                try:
                    if float(val) < 0.05:
                        run.bold = True
                except ValueError:
    ...
    doc.save(fname)
    print(f"Saved formatted table to {fname}")
